...
public class CitRoleSendInfectStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public CitRoleSendInfectStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","CheckForClose");
			    
			    
			    
			      // Receiving a message    
			      smf.add("CheckForClose", "waiting for CheckForClose");
			      
			      // Next states after receiving "CheckForClose"
			      
			       smf.add("waiting for CheckForClose","endCheckForClose");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "SendInfect","enable",options,this);
  ...
  if (this.isState("CheckForClose") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if ("1".equals("n")){
         try{
          Vector<AID> receivers=this.getActor("CoordRole");    
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"CheckForClose",
                   "SendInfect",optionsA,this,cardinality,0));
      }	 
      this.removeState("CheckForClose");
      this.addState("waiting for CheckForClose");
  } 
