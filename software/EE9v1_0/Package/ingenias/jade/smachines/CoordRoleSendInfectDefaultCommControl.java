...
import ingenias.exception.NotFound;
  ...
  public class CoordRoleSendInfectDefaultCommControl extends DefaultCommControl{
  
  	  private Vector<String> previous=new Vector<String>();
  	  
  public CoordRoleSendInfectDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  };
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLock("SendSickLocFF");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("CheckForClose") &&
    	 sb.isState("CheckForClose")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"SendSickLocFF").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("CheckForClose");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"SendSickLocFF");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLock("SendSickLocFF");
	       lockProcessed("SendSickLocFF");
           //MainInteractionManager.log("Removing lock SendSickLocFF",this.getAgent().getLocalName()+"-"+sb.getConversationID());
