...
public class CitRoleCoordCitInteractionStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public CitRoleCoordCitInteractionStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "InformReportFF");
			    
			    
			    
			     // States involved in message deliver
			     
			      smf.add("InformReportFF", "endInformReportFF");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
      ...
      actorsv.addAll(this.getActor("CoordRole"));
      {      
       Vector<AID> receivers=this.getActor("CoordRole");      
       for (AID aid:receivers){
        rolesv.add("CoordRole");
       }
      }
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","CoordCitInteraction",this.getAgentExternalDescription());
  ...
  if (this.isState("InformReportFF")) {
     ...
     try {
      ...
      if (this.getDCC().notifyMessageSent("InformReportFF",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "InformReportFF","CoordCitInteraction",this.getContentForNextMessage());
           getTimeout().stop();
      } else {
