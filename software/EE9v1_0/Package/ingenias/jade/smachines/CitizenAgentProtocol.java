...
public class CitizenAgentProtocol implements AgentProtocols{

public CitizenAgentProtocol(){};		
	...
	public ActiveConversation initialiseProtocols(String agentName,RuntimeConversation conv, 
        ...
        // Adds a protocol to the protocol queue
          StateBehavior   sb=null;
		   ...
		   if (conv.getInteraction().getId().equals("SendInfect") && conv.getPlayedRole().equals("CitRole")){
		   	DefaultCommControl dcc=new CitRoleSendInfectDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new CitRoleSendInfectStateBehavior(agentName,
				msr, msu,
				conv,                           		
				conv.getPlayedRole(),
			   	actors,
		   ...
		   if (conv.getInteraction().getId().equals("CoordCitInteraction") && conv.getPlayedRole().equals("CitRole")){
		   DefaultCommControl dcc=new CitRoleCoordCitInteractionDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new CitRoleCoordCitInteractionStateBehavior(
			agentName,
				msr,msu,
					conv,                           		
				   conv.getPlayedRole(),
	        ...
	        public DFAgentDescription[] getInteractionActors(String interaction, YellowPages yp) throws ingenias.jade.exception.NoAgentsFound{
              DFAgentDescription[] result=null;
               ...
               if (interaction.equals("CoordCitInteraction")){
                 ...
                 try {
                 String cardinality="";
		 DFAgentDescription[] agents=yp.getAgents("CoordRole");
                   if (agents.length<=0)
                      throw new ingenias.jade.exception.NoAgentsFound("Could not find an agent playing the role CoordRole");
